...
def task_update(
    ...
    failure_note: Optional[str] = typer.Option(None, "--failure-note", help="Concrete failure summary/evidence when status=failed or blocked"),
    failure_root_cause: Optional[str] = typer.Option(None, "--failure-root-cause", help="Root cause summary when status=failed or blocked"),
    failure_evidence: Optional[str] = typer.Option(None, "--failure-evidence", help="Concrete evidence when status=failed or blocked"),
    failure_recommended_next_owner: Optional[str] = typer.Option(None, "--failure-recommended-next-owner", help="Suggested next owner when status=failed or blocked"),
    failure_recommended_action: Optional[str] = typer.Option(None, "--failure-recommended-action", help="Suggested next action when status=failed or blocked"),
    ...
    triage_resolution_owner: Optional[str] = typer.Option(None, "--triage-resolution-owner", help="Explicit owner to resume after triage follow-up completion"),
    triage_resolution_action: Optional[str] = typer.Option(None, "--triage-resolution-action", help="Explicit action to resume after triage follow-up completion"),
    triage_resolution_note: Optional[str] = typer.Option(None, "--triage-resolution-note", help="Optional triage resolution note to persist"),
    ...
    request = TaskUpdateRequest(
        ...
        triage_resolution_owner=triage_resolution_owner,
        triage_resolution_action=triage_resolution_action,
        triage_resolution_note=triage_resolution_note,
    ...
    if effects.failure_notice is not None:
        data.update(effects.failure_notice)
    if effects.auto_releases:
        data["autoReleasedTasks"] = effects.auto_releases
    if effects.wake is None:
        def _human(d):
