...
def build_agent_prompt(
    ...
    lines.extend([
        ...
        "- For setup tasks, resolve the target repo first instead of hardcoding a remote: `python3 - <<'PY'` / `from pathlib import Path` / `from clawteam.workspace.git import probe_remote_head, resolve_remote_probe_target` / `target = resolve_remote_probe_target(Path('.'))` / `print(target)` / `print(probe_remote_head(Path('.'), remote=target.remote, branch=target.branch))`; fail closed if target resolution is ambiguous.",
        ...
        "- For setup tasks, if remote_status is `confirmed_latest`, the detached worktree must be created from that same confirmed target head; detached_head must equal remote_head or the setup should fail closed.",
