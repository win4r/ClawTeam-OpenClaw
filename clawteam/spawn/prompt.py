...
def build_agent_prompt(
    ...
    lines.extend([
        ...
        "- When you finish your current task, check your inbox for follow-up instructions BEFORE exiting:",
        f"  `clawteam inbox peek {team_name} --agent {agent_name}`",
        f"- If the inbox is empty (no more tasks), notify the leader and go idle:",
        f'  `clawteam inbox send {team_name} {leader_name} "Task done. Going idle, monitoring for follow-ups."`',
        ...
        "- When the leader explicitly shuts you down, type `exit` to terminate this session.",
        "",
        METACOGNITION_BLOCK,
        "",
    ])
